...
class Evaluator:
    def __init__(
        ...
        if hasattr(trainer, "linear_model"):
            self.sklearn_linear_model = trainer.linear_model
        else:
        ...
        for key in group_partition.keys():
            ...
            self.testloaders[key] = DataLoader(
                testset, batch_size=batch_size, sampler=sampler, num_workers=4, pin_memory=True, shuffle=False, worker_init_fn=seed_worker,
            )
        ...
        self.spurious_dataloader = DataLoader(spurious_dataset, batch_size=batch_size, num_workers=4, pin_memory=True, worker_init_fn=seed_worker,)
    ...
    def evaluate(self):
        ...
        self.accuracies = {}
        for key in tqdm(
            sorted(self.group_partition.keys()),
            "Evaluating group-wise accuracy",
        ):
